        ...
        for directories in input_list:
            ...
            for rt_tp in rt_dict.keys():
                ...
                if ('other_rtdose' in rt_dict[rt_tp].keys() and
                    ...
                    os.makedirs(other_dir)
                    for el in rt_dict[rt_tp]['other_rtdose']:
                        dir_name, el_name = el.split('/')[-2:]
                        if not os.path.isdir(os.path.join(other_dir, dir_name)):
                            os.makedirs(os.path.join(other_dir, dir_name))
                        shutil.copy2(el, os.path.join(other_dir, dir_name, el_name))
            if session_dict['RT']:
